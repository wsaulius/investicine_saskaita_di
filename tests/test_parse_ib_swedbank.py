...
def _write_swedbank_csv_fixture(tmp_path: Path) -> Path:
    csv_path = tmp_path / 'swedbank_fixture.csv'
    csv_path.write_text(
        '\n'.join([
            ...
            '"LT000000000000000001";"10";"2025-01-01";"";"Likutis pradžiai";"100.00";"EUR";"K";"";"AS";"";"";',
            '"LT000000000000000001";"20";"2025-01-10";"VARDENIS PAVARDENIS";"Pervedimas tarp savo sąskaitų";"900.00";"EUR";"K";"1";"MK";"";"";"";',
            '"LT000000000000000001";"20";"2025-02-12";"BROKER";"DIVIDENDAI / AGNC INVESTMENT CORP / US00123Q1040 /";"84.00";"USD";"K";"2";"MK";"";"";"";',
            '"LT000000000000000001";"20";"2025-02-13";"BROKER";"DIVIDENDAI / UBS MSCI EMU VALUE UCITS ETF / LU0446734369 /";"170.14";"EUR";"K";"3";"MK";"";"";"";',
            '"LT000000000000000001";"20";"2025-03-18";"Paysera LT, UAB";"R632041625 Uzsakymo nr: 90026 https://www.rontgen.lt projekte";"100.00";"EUR";"D";"4";"MK";"";"";"";',
            '"LT000000000000000001";"20";"2025-03-19";"PENSIJA";"III pakopos pensijų fondo įmoka";"100.00";"EUR";"D";"5";"MK";"";"";"";',
            '"LT000000000000000001";"20";"2025-12-31";"";"Valiutos keitimas, kursas 1.1994";"0.00";"EUR";"K";"6";"X";"";"";"";',
        ]) + '\n',
        encoding='utf-8',
    )
    return csv_path
...
def test_parse_swedbank_csv_real_statement_matches_expected_business_cases(tmp_path: Path) -> None:
    rules = load_swedbank_rules(str(ROOT / 'rules.yaml'))
    fixture_csv = _write_swedbank_csv_fixture(tmp_path)

    stmt = parse_swedbank_csv(str(fixture_csv), rules=rules)

    assert stmt.account_id == 'LT000000000000000001'
    ...
    assert len(stmt.deposits_withdrawals) == 2
    assert len(stmt.dividends) == 2
    assert sum(1 for row in stmt.deposits_withdrawals if row.amount > 0) == 2
    assert sum(1 for row in stmt.deposits_withdrawals if row.amount < 0) == 0
